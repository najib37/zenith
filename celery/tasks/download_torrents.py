    ...
    def post_process(self, movie_key, movie_info):
        match str(movie_info['type']):
            case 'video/x-matroska':
                converter = VideoConverter(movie_info['path'], '/home/data/movies/outs', movie_key)
                return converter
            case 'video/mp4':
                pass
            case _:
...
def process_jobs():
    ...
    while len(torrent_downloader.jobs) > 0:
        print("+++++++++++++++++++++++++++++++++++")
        print("dora dora dora")
        print("+++++++++++++++++++++++++++++++++++")
        for _, job in torrent_downloader.jobs.items():
            handler, converter = job.values()
            print("+++++++++++++++++++++++++++++++++++")
            print(f"progress {handler.status().progress}")
            print("+++++++++++++++++++++++++++++++++++")
            # if not converter.is_started and converter.is_ready_to_convert():
            #     print("+++++++++++++++++++++++++++++++++++")
            #     print("Starting conversion")
            #     print("+++++++++++++++++++++++++++++++++++")
            #     converter.start_conversion()
            #     # converter.create_playlist()
            #     converter.is_stated = True
            # if a == 0:
            #     job['converter'].start_conversion()
        # a += 1
        # if a > 10: break
        sleep(2)
